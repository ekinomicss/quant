...
def plot_skew(returns):
    """ A histogram of returns, skewness in the title. """
    num_columns = 5
...
def calc_summary_stats(returns, freq='daily'):
    ...
    if freq == 'daily':
        ...
        raise ValueError("Invalid frequency. Supported frequencies are 'daily', 'weekly' and 'monthly'")
    ...
    ann_return = returns.add(1).prod() ** (ann_factor) - 1
